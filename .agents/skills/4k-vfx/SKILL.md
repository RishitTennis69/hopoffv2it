...
## Prerequisites
pika MCP available, and **local `ffmpeg`** in the run environment (used to extract every frame and build the contact sheets). Two inputs from the user: **(1) a video clip** (a local file or a public URL) and **(2) the VFX change** they want made to it. Tools used: `mcp__plugin_pika_pika__upload_asset`, `mcp__plugin_pika_pika__probe_media`, `mcp__plugin_pika_pika__analyze_media`, `mcp__plugin_pika_pika__transcribe_audio`, `mcp__plugin_pika_pika__estimate_cost`, `mcp__plugin_pika_pika__generate_reference_video`, `mcp__plugin_pika_pika__task_status` (frame extraction + tiling is done locally with ffmpeg, not an MCP tool).
...
The 4K render is the expensive step, and once fired it can't be un-spent. **Never call `mcp__plugin_pika_pika__generate_reference_video` without explicit user approval in this session.** Present, in one message:
...
3. **The estimated cost** — call `mcp__plugin_pika_pika__estimate_cost` for the Step-7 `mcp__plugin_pika_pika__generate_reference_video` call and quote the result. If the estimate errors, say the cost is unknown and flag that 4K standard is the top-price tier — do not silently skip the number.
...
## Step 7 — Generate (Seedance 4K reference-to-video) (`mcp__plugin_pika_pika__generate_reference_video`)
Call `mcp__plugin_pika_pika__generate_reference_video` with the **exact** 4K recipe:
- `provider`: **`seedance`**
...
## Step 8 — Deliver
If the call returns inline, you have the 4K video URL. If it returns `{ task_id, status: running/background }`, poll `mcp__plugin_pika_pika__task_status(task_id)` in a tight loop until `completed` / `failed` / `cancelled`. On completion, return the **4K video URL** to the user. Note: the output CDN may be egress-blocked for in-session download — so deliver the URL itself rather than trying to fetch the bytes locally.
